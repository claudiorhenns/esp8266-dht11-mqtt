...
// credenciais de acesso ao mqtt
const char ssid* = "jenifer";
const char password* = "32481557" 
const char* mqtt_server = "test.mosquito.org"


// Configuração do pino do DHT11 (D4 corresponde ao GPIO2)
...
void dht_task(void *pvParameter) {
    while (1) {
        ...
        // Espera de 2 segundos entre leituras
        vTaskDelay(2000 / portTICK_PERIOD_MS);
    }
}

void app_main() {
    // Inicializa o sensor (caso seja necessário, dependendo da biblioteca)
...
//mosquitto_sub -h broker.hivemq.com -t "claudio_sensor_temperature" -p 1883
...
const char* mqtt_server = "mqtt://test.mosquitto.org"; // Broker MQTT alternativo
...
static void mqtt_event_handler(void* handler_args, esp_event_base_t base, int32_t event_id, void* event_data) {
    if (event_id == MQTT_EVENT_CONNECTED) {
        ...
        ESP_LOGI("MQTT", "Desconectado do servidor MQTT, tentando reconectar em 5 segundos...");
        vTaskDelay(5000 / portTICK_PERIOD_MS); // Delay de 5 segundos antes de tentar reconectar
...
void mqtt_init() {
    esp_mqtt_client_config_t mqtt_cfg = {
        .uri = mqtt_server,
        .port = 1883 // Porta padrão para MQTT sem TLS
    };
